...
def process_image(image_path):
    print(f"\n[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Starting to process scorecard image: {os.path.basename(image_path)}")
    ...
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Scorecard processing completed in {processing_time:.2f} seconds")
...
def process_new_images():
    ...
    if new_files:
        print(f"\n[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Detected {len(new_files)} new scorecard(s)")
    
    for fpath in new_files:
        if os.path.basename(fpath).lower().endswith(('.jpg', '.jpeg', '.png')):
            try:
                print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] New scorecard detected: {os.path.basename(fpath)}")
...
def main():
    global processed
    processed = set()
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Starting scorecard processing service...")
